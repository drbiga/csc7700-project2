...
def compute_score(
    ...
    pipeline_model_path = "spark/pipeline_model"
    if os.path.exists(pipeline_model_path):
        model = PipelineModel.load(pipeline_model_path)
    else:
        # If the pipeline model doesn't exist, we need to create it
        df = get_spark().read.option("multiline", True).json(INPUT_PATH)
        data = df.select(col(id_field).cast("string"), col(text_field)).na.drop()

        tokenizer = Tokenizer(inputCol=text_field, outputCol="tokens")
        remover = StopWordsRemover(inputCol="tokens", outputCol="filtered")
        hashing_tf = HashingTF(inputCol="filtered", outputCol="rawFeatures", numFeatures=1 << 18)
        idf = IDF(inputCol="rawFeatures", outputCol="tfidfFeatures")
    ...
    # Check if TF-IDF has been precomputed, if not, compute it
    if not os.path.exists(tfidf_df_path):
        df = get_spark().read.option("multiline", True).json(INPUT_PATH)
        data = df.select(col(id_field).cast("string"), col(text_field)).na.drop()
        ...
        # Save the TF-IDF vectors and pipeline model
        result = model.transform(data).select(col(id_field), col(text_field), col("tfidfFeatures"))
        result.write.mode("overwrite").parquet(tfidf_df_path)
        model.write().overwrite().save(pipeline_model_path)
